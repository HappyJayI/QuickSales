...
class InvoiceD(models.Model):     
    ...
    Qty = models.DecimalField(max_digits=19,decimal_places=0,blank=True,null=True,verbose_name="수량")
    ...
    Price = models.DecimalField(max_digits=19,decimal_places=0,blank=True,null=True,verbose_name="단가")
    Amt = models.DecimalField(max_digits=19,decimal_places=0,blank=True,verbose_name="금액")
    ...
    def save(self, *args, **kwargs):
        if (self.BasePrice is None ) or (self.BasePrice == 0):
            self.BasePrice = self.Price
        if (self.Amt is None ) or (self.Amt == 0):
            self.Amt = self.Qty * self.Price
            self.BaseAmt = self.Qty * self.BasePrice 
        super(InvoiceD, self).save(*args, **kwargs)
